...
void init_acpi() {
    ...
    RsdpDescriptor &rsdp_descriptor = *reinterpret_cast<RsdpDescriptor *>(rsd_ptr);
    ...
    if (rsdp_descriptor.revision > 0) {
        ...
        Rsdt &rsdt_header = *reinterpret_cast<Rsdt *>(static_cast<uint64_t>(rsdp_descriptor.rsdt_addr) + virtual_physical_base);
        Fadt &fadt = *find_facp(&rsdt_header);
...
Fadt *find_facp(Rsdt *rsdt) {
    ...
    for (uint32_t i = 0; i < entries; i++) {
        ...
        if (!strncmp(h->signature, "FACP", 4))
            return reinterpret_cast<Fadt *>(h);
    }
