...
namespace Spark {
    ...
    extern "C" void kernel_main(void* mb_info_ptr, uint32_t multiboot_magic) {
        if (multiboot_magic == 0x2BADB002 && mb_info_ptr) {
            Multiboot::Info& mb_info = *reinterpret_cast<Multiboot::Info*>(reinterpret_cast<uint64_t>(mb_info_ptr) + virtual_physical_base);  // Multiboot info
            Multiboot::MemoryMap* memory_map = reinterpret_cast<Multiboot::MemoryMap*>(static_cast<uint64_t>(mb_info.mmap_addr));             // Memory map array
            ...
            if (Vmm::map_pages(Vmm::get_current_context(), virtual_fb, reinterpret_cast<void*>(mb_info.framebuffer_addr), (mb_info.framebuffer_width * mb_info.framebuffer_pitch + page_size - 1) / page_size, Vmm::VirtualMemoryFlags::VMM_WRITE | MemoryFlags::READ)) {
                ...
                if (pat_supported)
                    Vmm::update_perms(Vmm::get_current_context(), virtual_fb, (mb_info.framebuffer_width * mb_info.framebuffer_pitch + page_size - 1) / page_size, Vmm::VirtualMemoryFlags::VMM_WRITE | MemoryFlags::READ | Vmm::VirtualMemoryFlags::VMM_LARGE | Vmm::VirtualMemoryFlags::VMM_WT);  // Since PAT MSR is supported, we can update the permissions of the maps to use PAT
        ...
        while (1)
            asm volatile(
                "sti\n\t"
                "hlt");  // Halt CPU so we don't hug its usage
    }
}  // namespace Spark