...
inline Spark::Acpi::SdtHeader* bios_rsdt_search(const char* signature) {
    if (!rsdp_info.version)
        return nullptr;

    if (rsdp_info.version >= 2) {
        Spark::Acpi::Xsdt* xsdt = (Spark::Acpi::Xsdt*)rsdp_info.address;
        ...
        Spark::Acpi::Rsdt* rsdt = (Spark::Acpi::Rsdt*)rsdp_info.address;
...
void Spark::Acpi::init() {
    ...
    sprintf(text, "[ACPI] Starting up for v%d...", rsdp_info.version, rsdp_info.oem_id);
    Terminal::write_line(text, 0xFFFFFF);

    Apic::LocalApic::init();
    madt = (Madt*)bios_rsdt_search("APIC");
