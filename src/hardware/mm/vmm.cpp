...
int map_address(uint64_t *pml4, uint16_t flags, uint64_t physical_address, size_t address) {
    ...
    if (pml4[pml4_index] & 0x1)
        pml3 = (uint64_t *)((pml4[pml4_index] & 0xFFFFFFFFFFFFF000) + KERNEL_VMA);
    else {
        pml3 = (uint64_t *)((size_t)calloc(1) + KERNEL_VMA);
        if ((size_t)pml3 == KERNEL_VMA) goto fail1;
        pml4[pml4_index] = *pml3 | 0b111;
    }
    if (pml3[pml3_index] & 0x1)
        pml2 = (uint64_t *)((pml3[pml3_index] & 0xFFFFFFFFFFFFF000) + KERNEL_VMA);
    else {
        pml2 = (uint64_t *)((size_t)calloc(1) + KERNEL_VMA);
        if ((size_t)pml2 == KERNEL_VMA) goto fail2;
        pml3[pml3_index] = *pml2 | 0b111;
    }
    if (pml2[pml2_index] & 0x1)
        pml1 = (uint64_t *)((pml2[pml2_index] & 0xFFFFFFFFFFFFF000) + KERNEL_VMA);
    else {
        pml1 = (uint64_t *)((size_t)calloc(1) + KERNEL_VMA);
        if ((size_t)pml1 == KERNEL_VMA) goto fail3;
